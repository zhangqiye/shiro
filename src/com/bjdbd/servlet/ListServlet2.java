	...
	protected void service( HttpServletRequest request , HttpServletResponse response ) 
		...
		if(name == null || name == ""){
			String SQL = "select id,office_id,finance_id,login_name,no,name,email,"
					+ "phone,mobile,password,user_type,login_flag from sys_user where del_flag = 0 and user_type='3' and office_id='"+u.getOfficeId()+"' limit ?,? ";
			String CSQL = "select count(id) from sys_user where del_flag=0 and user_type='3' and office_id='"+u.getOfficeId()+"'";
...
/*					+ "<th>用户类型</th>"*/
			...
			for(User pojo : userlist){
				...
				sb.append("<td><a class='button border-red' href='javascript:;' onclick='goDel(\"" + str + "\")'>删除用户</a>"
						+ "&nbsp;&nbsp;&nbsp;&nbsp;"
				);
				if(pojo.getLoginFlag() == "0"||pojo.getLoginFlag().equals("0")){
					sb.append("<a class='button border-main' href='javascript:;' onclick='goAllow(\"" + str + "\")'>允许登录</a>" 
							+"</tr>");
				}
				if(pojo.getLoginFlag() == "1"||pojo.getLoginFlag().equals("1")){
					sb.append("<a class='button border-red' href='javascript:;' onclick='goStop(\"" + str + "\")'>禁止登录</a>" 
							+"</tr>");
				}
			...
			String SQL = "select id,office_id,finance_id,login_name,no,name,email,"
					...
					+ "where del_flag = 0 and user_type='3' and office_id='"+u.getOfficeId()+"' and name LIKE '%"+name+"%' limit ?,? ";
			String CSQL = "select count(*) from sys_user where del_flag=0 and name LIKE '%"+name+"%' and user_type='3' and company_id='"+u.getCompanyId()+"'";
