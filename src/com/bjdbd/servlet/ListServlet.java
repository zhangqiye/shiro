	...
	protected void service( HttpServletRequest request , HttpServletResponse response ) 
		...
		if(name == null || name == ""){
			String SQL = "select id,office_id,finance_id,login_name,no,name,email,"
					+ "phone,mobile,password,user_type,login_flag from sys_user where del_flag = 0 and user_type='2' and office_id='"+u.getOfficeId()+"' limit ?,? ";
			String CSQL = "select count(id) from sys_user where del_flag=0 and user_type='2' and office_id='"+u.getOfficeId()+"'";
...
/*					+ "<th>用户类型</th>"*/
			...
			for(User pojo : userlist){
				...
				sb.append("<td><a href='javascript:;' class='button border-main' onclick='goUpdate(\"" + str + "\")'>修改信息</a>"
						+ "&nbsp;&nbsp;&nbsp;&nbsp;"
						+ "<a class='button border-red' href='javascript:;' onclick='goDel(\"" + str + "\")'>移除财务</a>"
						+ "&nbsp;&nbsp;&nbsp;&nbsp;"
				);
				/*if(pojo.getUserType() == "2"||pojo.getUserType().equals("2")){
			...
			String SQL = "select id,office_id,finance_id,login_name,no,name,email,"
					...
					+ "where del_flag = 0 and user_type='2' and office_id='"+u.getOfficeId()+"' and name LIKE '%"+name+"%' limit ?,? ";
			String CSQL = "select count(*) from sys_user where del_flag=0 and name LIKE '%"+name+"%' and user_type='2' and company_id='"+u.getCompanyId()+"'";
			...
			for(User pojo : userlist){
				...
				sb.append("<tr>" +
						...
						"<td><a href='javascript:;' class='button border-main' onclick='goUpdate(\"" + str + "\")'>修改信息</a>"
								+ "&nbsp;&nbsp;&nbsp;&nbsp;"
								+ "<a class='button border-red' href='javascript:;' onclick='goDel(\"" + str + "\")'>移除财务</a>"
								+ "&nbsp;&nbsp;&nbsp;&nbsp;"
								);
